...
const SearchBox = () => {
  ...
  const fetchLocationData = async (query = '') => {
    ...
    const response = await fetch(
      `${SEARCH_LOCATIONS_API_BASE_URL}${encodeURIComponent(
        query
      )}${SEARCH_LOCATIONS_API_DEFAULT_PARAMS}`
    );
  ...
  const getNearestProvidersFromLocation = (_lat, _lng, name) => {
    const segmentedProvidersIds = providersDataset.reduce((acc, provider) => {
    ...
    if (segmentedProvidersIds.length === 0)
      dispatch({
        type: SET_ERRORS,
        payload: { search: "Sorry, we couldn't find providers in that area." },
      });
  ...
  const locationsOptions =
    ...
    locations.map(({ place_id, lat, lon, display_name }) => (
      <div
        ...
        onClick={() => getNearestProvidersFromLocation(lat, lon, display_name)}
      >
  ...
  return (
    ...
    >
      <form
        className={classNames('bg-white shadow-md flex px-4 py-2 gap-4', {
          'rounded-md': locations.length === 0 || !showDropdown,
          'rounded-tl-md rounded-tr-md': locations.length > 0 && showDropdown,
        })}
      ...
      <div
        className={classNames(
          ...
          {
            '-translate-y-8 opacity-0 pointer-events-none': !showDropdown,
